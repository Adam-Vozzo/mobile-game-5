// ============================================================================
// achievement-viewer.js — Achievements viewer for "Make the Cat Dizzy!"
// Self-contained module. Adds a top-center trophy button that opens a panel
// listing all 11 achievements. Reads unlocked IDs from localStorage
// "mtcd_achievements" (array, written by achievements.js).
// ============================================================================
(function () {
    'use strict';
    ...
    // Mirror of achievements.js IDs/metadata (achievements.js doesn't expose
    ...
    const size = () => { const s = MTCD.canvasSize(); return { W: s.W || window.innerWidth || 800, H: s.H || window.innerHeight || 600 }; };
    ...
    function roundedPath(c, x, y, w, h, r) {
        ...
        c.arcTo(x + w, y, x + w, y + rr, rr); c.lineTo(x + w, y + h - rr);
        c.arcTo(x + w, y + h, x + w - rr, y + h, rr); c.lineTo(x + rr, y + h);
        c.arcTo(x, y + h, x, y + h - rr, rr); c.lineTo(x, y + rr);
        c.arcTo(x, y, x + rr, y, rr); c.closePath();
    }
    ...
    function clickSfx() {
        try { const A = MTCD.audio && MTCD.audio(); if (A && A.SFX && A.SFX.click) A.SFX.click(); }
        catch (e) { /* ignore */ }
    }

    function open() {
        if (ui.open) return;
        ui.open = true; ui.openT = 0;
    ...
    function drawToggleButton(c) {
        const { W } = size();
        const r = 18;
        const cx = W * 0.5, cy = 80;
        ui.toggleRect = { x: cx - r, y: cy - r, w: r * 2, h: r * 2 };

        c.save();
        c.shadowColor = 'rgba(0,0,0,0.45)';
        c.shadowBlur = 10; c.shadowOffsetY = 2;
    ...
    function drawPanel(c) {
        ...
        const pw = Math.min(620, W * 0.88), ph = Math.min(520, H * 0.88);
        ...
        c.fillStyle = grd;
        roundedPath(c, px, py, pw, ph, 18); c.fill();
        c.shadowColor = 'transparent'; c.shadowBlur = 0; c.shadowOffsetY = 0;
        c.strokeStyle = 'rgba(255,255,255,0.10)'; c.lineWidth = 1;
        roundedPath(c, px + 0.5, py + 0.5, pw - 1, ph - 1, 18); c.stroke();
        ...
        const closeSize = 40, cbx = px + pw - closeSize - 14, cby = py + 14;
        ui.closeRect = { x: cbx, y: cby, w: closeSize, h: closeSize };
        c.fillStyle = 'rgba(255, 60, 100, 0.18)';
        roundedPath(c, cbx, cby, closeSize, closeSize, 12); c.fill();
        c.strokeStyle = 'rgba(255, 100, 130, 0.55)'; c.lineWidth = 1;
        roundedPath(c, cbx + 0.5, cby + 0.5, closeSize - 1, closeSize - 1, 12); c.stroke();
        c.fillStyle = '#ffffff';
        ...
        const gridX = px + 22, gridYTop = py + 92, gridW = pw - 44, gap = 12;
        const cardW = (gridW - gap * (cols - 1)) / cols, cardH = 70;
        c.save();
        roundedPath(c, gridX - 4, gridYTop - 4, gridW + 8, ph - (gridYTop - py) - 18, 12);
        c.clip();
        for (let i = 0; i < ACHIEVEMENTS.length; i++) {
            const col = i % cols, row = (i / cols) | 0;
            drawCard(c, gridX + col * (cardW + gap), gridYTop + row * (cardH + gap),
    ...
    function drawCard(c, x, y, w, h, ach, isUnlocked) {
        ...
        grd.addColorStop(0, isUnlocked ? 'rgba(60, 44, 96, 0.85)' : 'rgba(28, 22, 46, 0.85)');
        grd.addColorStop(1, isUnlocked ? 'rgba(30, 22, 54, 0.85)' : 'rgba(16, 12, 30, 0.85)');
        c.fillStyle = grd;
        roundedPath(c, x, y, w, h, 12); c.fill();
        c.lineWidth = 1;
        c.strokeStyle = isUnlocked ? 'rgba(255, 216, 77, 0.32)' : 'rgba(255, 255, 255, 0.06)';
        roundedPath(c, x + 0.5, y + 0.5, w - 1, h - 1, 12); c.stroke();

        const iconR = 22, iconCx = x + iconR + 12, iconCy = y + h * 0.5;
        ...
        c.lineWidth = 1.25;
        c.beginPath(); c.arc(iconCx, iconCy, iconR - 0.6, 0, Math.PI * 2); c.stroke();
        ...
        if (!isUnlocked) {
            const bx = iconCx + iconR - 8, by = iconCy + iconR - 8;
            ...
            c.beginPath(); c.arc(bx, by, 8.5, 0, Math.PI * 2); c.stroke();
            c.font = '600 11px system-ui, -apple-system, "Segoe UI", Roboto, sans-serif';
            c.fillStyle = 'rgba(255,255,255,0.72)';
            c.fillText('🔒', bx, by + 1);
        }

        const textX = iconCx + iconR + 12, textW = (x + w) - textX - 12;
        c.textAlign = 'left'; c.textBaseline = 'top';
